...
// グローバル変数
//*****************************************************************************
LPDIRECT3DTEXTURE9		D3DTextureBeam = NULL;			// テクスチャへのポリゴン
...
HRESULT InitBeam(int type)
{
	...
	{		
		...
		beam->Texture = D3DTextureBeam;					// テクスチャ情報
...
{
	if (D3DTextureBeam != NULL)	
	{	// テクスチャの開放
		D3DTextureBeam->Release();
		D3DTextureBeam = NULL;
	}	
}
//=============================================================================
...
void UpdateBeam(void)
{
	...
	for (int i = 0; i < BEAM_MAX; i++, beam++)
	{
		if (beam->use)			// 未使用状態のBeamを見つける
		{
			...
			beam->abs_pos.x -= BEAM_SPEED * (sinf(beam->BaseAngle + beam->rot.z) - cosf(beam->BaseAngle + beam->rot.z));			
			beam->abs_pos.y += BEAM_SPEED * (cosf(beam->BaseAngle + beam->rot.z) + sinf(beam->BaseAngle + beam->rot.z));
			// 画面外まで進んだ？
			if (beam->pos.y < -TEXTURE_BEAM_SIZE_Y || 
				beam->pos.y > SCREEN_HEIGHT		   ||
				beam->pos.x < -TEXTURE_BEAM_SIZE_X ||
				beam->pos.x > SCREEN_WIDTH
				)
			{
				beam->use = false;
			}

			//beam->pos.x -=  GetPlayer(0)->pos.x / 4.0f;
			SetVertexBeam(i);
		}
	}
}
...
void SetVertexBeam(int no)
{
	...
	// 頂点座標の設定	
	beam->pos.y = beam->abs_pos.y;
	beam->pos.x = beam->abs_pos.x - GetPlayer(0)->pos.x / 4.0f;
