...
HRESULT InitBeam(int type)
{
	...
	BEAM *beam = &beamWk[0];		// エネミーのポインターを初期化
...
void UpdateBeam(void)
{
	...
	BEAM *beam = beamWk;			// バレットのポインターを初期化

	for (int i = 0; i < BEAM_MAX; i++, beam++)
	{
		if (beam->use == true)			// 未使用状態のバレットを見つける
		{
			// バレットの移動処理
...
void SetVertexBeam(int no)
{
	BEAM *beam = &beamWk[no];			// バレットのポインターを初期化
...
{
	BEAM *beam = &beamWk[0];			// バレットのポインターを初期化

	// もし未使用の弾が無かったら発射しない( =これ以上撃てないって事 )
	for (int i = 0; i < BEAM_MAX; i++, beam++)
	{
		if (beam->use == false)			// 未使用状態のバレットを見つける
		{
